...
const AskModal = ( {isOpen, onClose, onSubmit} ) => {
    ...
    return (
        <div className="fixed inset-0 bg-violet-500/20 flex items-center justify-center z-50" onClick={onClose}>
            <div className="bg-black w-full fixed max-w-xl max-h-[600px] overflow-y-auto p-3 rounded-lg shadow-lg transform scale-95 opacity-90 transition-all duration-300 ease-in-out animate-[fadeIn_0.3s_ease-out_forwards]" onClick={(e) => e.stopPropagation()}>
                <IoClose 
                    onClick={onClose}
                ...
                />
                    <img src={user.avatar} alt={user.name} className="w-8 h-8 ml-5 rounded-full"/>
                        <textarea
                            ...
                            className="w-full p-4 ml-3 mt-1.5 resize-none border-none outline-none text-white font-light max-h-[400px] placeholder-white/35 max-row-[5] text-justify"
